...
$solProblem = trim($paramValues['solProblem']);
$solution = trim($paramValues['sol']);
$solRecomend = trim($paramValues['solRecomend']);
...
// Получаем заявку с проверкой прав доступа
	$req = $db->prepare("SELECT DISTINCT `rq`.`guid` AS `guid` ".
            				...
            				"JOIN `contractDivisions` AS `div` ON `rq`.`onWait` = 0 AND `rq`.`id` = :requestId ".
            					"AND `rq`.`currentState` IN ('fixed','accepted') AND `div`.`guid` = `rq`.`contractDivision_guid` ".
...
try {
	$soapReq = array('sd_requestevent_table' => array(array('CodeNodeSiteSD' => 'SDTEST2',
															...
															'timestamp' 	 => $soapTime,
															'user_guid' 	 => $userGuid,
															'solutionProblem'=> $solProblem,
															'solution'		 => $solution, 
															'solutionRecomendation'=> $solRecomend,
															'comment' 		 => "Проблема: {$solProblem}\nРешение: {$solution}\nРекомендации: {$solRecomend}")));
...
try {
	...
	$req = $db->prepare("UPDATE `requests` SET `currentState` = 'repaired', `stateChangedAt` = :stateChangedAt, ".
												"`repairedAt` = :stateChangedAt, `fixedAt` = IFNULL(`fixedAt`, :stateChangedAt), ".
												"`solutionProblem` = :solProblem, `solution` = :solution, ".
												"`solutionRecomendation` = :solRecomend ".
							"WHERE `id` = :requestId");
	$req->execute(array('stateChangedAt' => $time, 'requestId' => $paramValues['id'], 'solProblem' => $solProblem, 
						'solution' => $solution, 'solRecomend' => $solRecomend));
	$req = $db->prepare("INSERT INTO `requestEvents` (`timestamp`, `event`, `newState`, `request_guid`, `user_guid`) ".
    					"VALUES (:stateChangedAt, 'changeState', 'repaired', UNHEX(REPLACE(:requestGuid, '-', '')), ".
