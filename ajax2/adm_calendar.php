<?php
	...
	switch($paramValues['call']) {
		case 'init':
			...
			break;
		case 'setYear':
			if (!isset($paramValues['year'])) {
				returnJson(array('error' => 'Ошибка в параметрах.'));
				exit;
			}
			$year = min(max(intval($paramValues['year']), 2014), 2029);
			break;
		case 'change':
			if (!isset($paramValues['year']) || !isset($paramValues['month']) || !isset($paramValues['day'])) {
				returnJson(array('error' => 'Ошибка в параметрах.'));
				exit;
			}
			$date = "{$paramValues['year']} {$paramValues['month']} {$paramValues['day']}";
