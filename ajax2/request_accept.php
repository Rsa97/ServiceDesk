...
// Получаем список заявок с проверкой прав доступа
	$req = $db->prepare("SELECT DISTINCT `rq`.`id` AS `id`, `rq`.`guid` AS `guid` ".
            				...
            				"JOIN `contractDivisions` AS `div` ON `rq`.`onWait` = 0 AND FIND_IN_SET(`rq`.`id`, :list) ".
            					"AND `rq`.`currentState` IN ('received', 'preReceived') AND `div`.`guid` = `rq`.`contractDivision_guid` ".
...
foreach($list as $id => $guid) {
	...
	try {
		...
		$req = $db->prepare("UPDATE `requests` SET `currentState` = 'accepted', `stateChangedAt` = :stateChangedAt, ".
													"`reactedAt` = :stateChangedAt, ".
      												"`engineer_guid` = UNHEX(REPLACE(:userGuid, '-', '')) ".
								"WHERE `id` = :requestId");
		$req->execute(array('stateChangedAt' => $time, 'requestId' => $id, 'userGuid' => $userGuid));
		$req = $db->prepare("INSERT INTO `requestEvents` (`timestamp`, `event`, `newState`, `request_guid`, `user_guid`) ".
								"VALUES (:stateChangedAt, 'changeState', 'accepted', UNHEX(REPLACE(:requestGuid, '-', '')), ".
