<?php
	...
	switch($paramValues['call']) {
		...
		case 'update':
			if (!isset($paramValues['id']) || ($id = $paramValues['id']) < 0 ||
				...
				!isset($paramValues['shortName']) || ($shortname = $paramValues['shortName']) == '') {
				returnJson(array('error' => 'Ошибка в параметрах.'));
				exit;
			}
			if ($id == 0) {
		...
		case 'setCheck':
			if (!isset($paramValues['id']) || ($id = $paramValues['id']) <= 0 ||
				!isset($paramValues['field']) || ($field = $paramValues['field']) != 'utility' ||
				!isset($paramValues['value']) || (($val = $paramValues['value']) != 0 && $val != 1)) {
				returnJson(array('error' => 'Ошибка в параметрах.'));
				exit;
			}
	...
	$req = $mysqli->prepare("SELECT `s`.`guid`, `s`.`name`, `s`.`shortname`, `s`.`utility`, ".
								"IFNULL(`cs`.`service_guid`, IFNULL(`dss`.`service_guid`, `r`.`service_guid`)) ".
								"FROM `services` AS `s` ".
								"LEFT JOIN (SELECT DISTINCT `service_guid` FROM `contractServices`) AS `cs` ON `cs`.`service_guid` = `s`.`guid` ".
								"LEFT JOIN (SELECT DISTINCT `service_guid` FROM `divServicesSLA`) AS `dss` ON `dss`.`service_guid` = `s`.`guid` ".
								"LEFT JOIN (SELECT DISTINCT `service_guid` FROM `requests`) AS `r` ON `r`.`service_guid` = `s`.`guid` ".
								"ORDER BY `s`.`name`");
