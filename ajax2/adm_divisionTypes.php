<?php
	...
	switch($paramValues['call']) {
		...
		case 'update':
			...
			$comment = (isset($paramValues['comment']) ? $paramValues['comment'] : '');
			if ($id == 0) {
	...
	$req = $mysqli->prepare("SELECT `dt`.`guid`, `dt`.`name`, `dt`.`comment`, `cd`.`type_guid` ".
								"FROM `divisionTypes` AS `dt` ".
								"LEFT JOIN (SELECT DISTINCT `type_guid` FROM `contractDivisions`) AS `cd` ON `cd`.`type_guid` = `dt`.`guid` ".
								"ORDER BY `name`");
