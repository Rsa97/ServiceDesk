...
// Проверка прав
	...
	$req->execute(array('divisionGuid' => $paramValues['division'], 'serviceGuid' => $paramValues['service'],
						'slaLevel' => $paramValues['slaLevel'], 'contactGuid' => null, 'equipmentGuid' => $equipment));
	$ok = 0;
...
if ($ok == 0) {
	return_format($format, array('state' => 'error', 'text' => 'Ошибка в параметрах')); 
	exit;
}
...
try {
	...
	$req->execute(array('problem' => $paramValues['problem'], 'createdAt' => $time['createdAt'], 'reactBefore' => $time['reactBefore'], 
						'fixBefore' => $time['fixBefore'], 'repairBefore' => $time['repairBefore'], 'contactGuid' => $userGuid, 
...
if ($req->rowCount() == 0) {
	return_format($format, array('state' => 'error', 'text' => 'Внутренняя ошибка сервера'));
  	exit;
}
...
if (false === $soap) {
	return_format($format, array('state' => 'error', 'text' => 'Нет связи с 1С'));
	exit;
}
try {
	...
	return_format($format, array('state' => 'error', 'text' => 'Внутренняя ошибка сервера (SOAP)', 
...
try {
	...
	$req->execute(array('createdAt' => $time['createdAt'], 'requestGuid' => $answer->GUID, 'contactGuid' => null));
	$db->query('COMMIT');
} catch (Exception $e) {
	return_format($format, array('state' => 'error', 'text' => 'Внутренняя ошибка сервера (MySQL)', 
								 'orig' => 'MySQL error '.$e->getMessage()));
