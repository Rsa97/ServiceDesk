<?php
/**
 * Получить данные подразделений по guid
 * 
 * PHP version 5
 * 
 * @category Divisions
...
try {
    $req = $db->prepare(
        "SELECT HEX(`guid`), `name`, `email`, `phone`, `address`, `yurAddress`, "
        .       "HEX(`contract_guid`), HEX(`contragent_guid`), HEX(`engineer_guid`) "
    ...
    while ($row = $req->fetch(PDO::FETCH_NUM)) {
        list(
            $guid, $name, $email, $phone, $address, $yurAddress, $contractGuid, 
            $contragentGuid, $engineerGuid
        ) = $row;
        $result[$guid] = array(
            'name' => $name,
            'contacts' => array(
            ...
            'contract' => ('' == $contractGuid ? null : $contractGuid),
            'contragent' => ('' == $contragentGuid ? null : $contragentGuid),
