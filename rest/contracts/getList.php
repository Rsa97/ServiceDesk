<?php
/**
 * Получить данные контрактов по guid
 * 
 * PHP version 5
 * 
 * @category Contracts
...
try {
    $req = $db->prepare(
        "SELECT HEX(`guid`), `number`, `email`, `phone`, `address`, `yurAddress`, HEX(`contragent_guid`) "
        .   "FROM `contracts` "
    ...
    while ($row = $req->fetch(PDO::FETCH_NUM)) {
        list($guid, $number, $email, $phone, $address, $yurAddress, $contragentGuid) = $row;
        $result[$guid] = array(
            'number' => $number,
            'contacts' => array(
                'email' => $email,
