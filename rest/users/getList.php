<?php
/**
 * Получить данные пользователей по guid
 * 
 * PHP version 5
 * 
 * @category Users
...
try {
    $req = $db->prepare(
        "SELECT HEX(`guid`), `lastName`, `firstName`, `middleName`, `email`, `phone`, `address` "
        .   "FROM `users` "
    ...
    while ($row = $req->fetch(PDO::FETCH_NUM)) {
        list($guid, $lastName, $firstName, $middleName, $email, $phone, $address) = $row;
        $result[$guid] = array(
