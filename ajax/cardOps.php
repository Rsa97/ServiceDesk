	...
	if (isset($_REQUEST['op']))
		switch ($_REQUEST['op']) {
			...
			case 'serviceList':
				...
				$partner = '';
				$partnerAddress = '';
				$req = $mysqli->prepare("SELECT `p`.`name`, `p`.`address` ".
											"FROM `partner` AS `p` ".
											"JOIN `users` AS `u` ON `p`.`id` = `u`.`partner_id` ".
											"WHERE `u`.`id` = ?");
				$req->bind_param('i', $userId);
				$req->bind_result($partner, $partnerAddress);
				if (!$req->execute()) {
					sendJson(array('error' => 'Внутренняя ошибка сервера.'));
					exit;
				}
				...
				$template = preg_replace('/#requestNumber#/', $requestNum, $template);
				$template = preg_replace('/#number#/', $requestNum.'-01', $template);
				$template = preg_replace('/#engeneer#/', rtf_encode($engLN.' '.$engGN.' '.$engMN), $template);
				$engShort = $engLN;
				if ($engGN != '') {
					$engShort .= (' '.mb_substr($engGN, 0, 1, 'utf-8').'.');
					if ($engMN != '')
						$engShort .= (' '.mb_substr($engMN, 0, 1, 'utf-8').'.');
				}
				$template = preg_replace('/#engeneerShort#/', rtf_encode($engShort), $template);
				$template = preg_replace('/#partner#/', rtf_encode($partner), $template);
				...
				$template = preg_replace('/#client#/', rtf_encode($contLN.' '.$contGN.' '.$contMN), $template);
