<?php
	...
	$_SESSION['time'] = time();
	session_commit();
	if (!isset($_REQUEST['call']) || !isset($_REQUEST['contId']) || ($contId = $_REQUEST['contId']) <= 0) {
	...
	switch($_REQUEST['call']) {
		...
		case 'update':
			if (!isset($_REQUEST['id']) || ($id = $_REQUEST['id']) < 0 ||
				...
				!isset($_REQUEST['divType']) || ($divType = $_REQUEST['divType']) <= 0 ||
				!isset($_REQUEST['SLA']) || 
				!isset($_REQUEST['toReact']) || ($toReact = str_to_minutes($_REQUEST['toReact'])) < 0 ||
				!isset($_REQUEST['toFix']) || ($toFix = str_to_minutes($_REQUEST['toFix'])) < 0 ||
				!isset($_REQUEST['toRepair']) || ($toRepair = str_to_minutes($_REQUEST['toRepair'])) < 0 || 
				!isset($_REQUEST['quality']) || ($quality = $_REQUEST['quality']) < 0 || $quality > 100 ||
				!isset($_REQUEST['dayStart']) || ($dayStart = asTime($_REQUEST['dayStart'])) == '' || 
				!isset($_REQUEST['dayEnd']) || ($dayEnd = asTime($_REQUEST['dayEnd'])) == '') {
				returnJson(array('error' => 'Ошибка в параметрах.'));
				exit;
			}
			...
			if ($id == 0) {
				$req = $mysqli->prepare("INSERT IGNORE INTO `divServicesSLA` (`contract_id`, `service_id`, `divType_id`, `slaLevel`, ".
												...
												"VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
				$req->bind_param('iiisiiiiss', $contId, $servId, $divType, $_REQUEST['SLA'], $toReact, $toFix, $toRepair, $quality, 
												$dayStart, $dayEnd);
			} else {
				$req = $mysqli->prepare("UPDATE IGNORE `divServicesSLA` SET `service_id` = ?, `divType_id` = ?, `slaLevel` = ?, ".
												...
												"`endDayTime` = ? ".
												"WHERE `id` = ?");
				$req->bind_param('iisiiiissi', $servId, $divType, $_REQUEST['SLA'], $toReact, $toFix, $toRepair, $quality, 
												$dayStart, $dayEnd, $id);
			}
			if (!$req->execute()) {
				returnJson(array('error' => 'Внутренняя ошибка сервера.'));
		...
		case 'setCheck':
			if (!isset($_REQUEST['id']) || ($id = $_REQUEST['id']) < 0 || 
			!isset($_REQUEST['value']) || (($value = $_REQUEST['value']) != 0 && $value != 1) ||
	...
	$req = $mysqli->prepare("SELECT `dss`.`id`, `s`.`shortname`, `dt`.`name`, `dss`.`slaLevel`, FIND_IN_SET('work', `dayType`) != 0, ".
								...
								"JOIN `divisionTypes` AS `dt` ON `dt`.`id` = `dss`.`divType_id` ".
								"WHERE `dss`.`contract_id` = ? ".
								"ORDER BY `s`.`shortname`, `dt`.`name`, `dss`.`slaLevel`");
	$req->bind_param('i', $contId);
